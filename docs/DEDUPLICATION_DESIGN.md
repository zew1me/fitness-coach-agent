...
state that would create a chain or cycle.

**`p_surviving_id` is validated, not trusted.** First-created survivorship is the
document's own invariant, and leaving it to the caller would make it exactly the
kind of application assumption this section exists to remove — the more so
because the athlete-authored tie-break below "prefers the surviving row, whose
first-created identity is stable," which is unsound if a newer row can be named
the survivor. So under the row locks the RPC decides survivorship itself, using
the same total order the field reconciler uses: earlier `created_at`, then
lexicographically smaller `id` when the timestamps are equal. A reversed
selection raises `22023` rather than being silently honoured. Repeating the exact
completed merge is
...
proposal's `expected_phrase`.

**Consent is group-scoped, and outlives the first pairwise merge.** A Tier-B
group of N members is N-1 transactions (see the orchestration contract below),
so consuming the proposal in the first one would strand the rest: the athlete
confirmed _this group_, once, and would have to re-confirm a group they had
already approved. Instead the proposal stays pending and is bound to the sorted
member id list. Each transaction in the sequence verifies it, marks that pair
merged against it, and leaves it live; the final pair sets `consumed_at`, in the
same transaction as the last merge. A proposal is therefore consumed exactly
once, for exactly the group the athlete named. The confirmation-message check is
re-verified on every transaction rather than trusted from the first, so a
concurrent expiry, a replay, or a changed group still halts the sequence
mid-way — and a halted sequence is the consistent partially-merged state the
orchestration contract already covers. Member versions are re-read per
transaction, as they must be; only the _authorization_ spans the sequence, never
the version map. A model-
...
clears the merged-away row's dedup state, restores any audited plan link, retires
its audit entry, and recomputes the surviving row's fields. A false-positive merge
costs the athlete one conversation, not a workout.

**"Retires", not "deletes" — the merge history outlives the merge.** The entry
leaves `raw_extraction.merged_from`, because that array is the live member list
the reconciler reads and a reversed merge is no longer a member. It is appended
to `raw_extraction.merge_history` on the same row, in the same transaction,
stamped with the unmerge time and actor. Dropping it outright would make an
unmerge the one operation in this design that destroys evidence — and the merge
it erased is exactly the record you need when an athlete asks why their numbers
moved twice, or when a bad Tier-A rule has to be reconstructed after the fact.
`merge_history` is append-only: nothing in this design rewrites or removes an
entry once written. It is provenance, never an input to reconciliation, so it
cannot affect the merged values the way a stale `merged_from` entry would.
...
`{activity_id: updated_at}` map with the reconciled fields. **"Complete group"
means the survivor, every member still active, and every member already merged
away into that survivor** — not just the two rows named in the call. Scoping the
map to active rows only would let a concurrent unmerge of an earlier member slip
past unnoticed, which is exactly the contribution re-derivation depends on. Under
its row locks, the RPC compares that map with the exact current member set and
versions. A missing, added, or changed member raises serialization failure `40001`
without writing. The service then re-reads, re-derives, and retries a bounded
number of times. Thus policy stays in Python while the database proves that the
policy ran
on the state it is atomically replacing.

**A group of N members is N−1 sequential `merge_activity` transactions, not one.**
Each call is individually atomic and idempotent, and the complete-group version
map above is what makes the sequence safe rather than merely convenient. Because
...
| RPC                                                                                                                                      | Behaviour                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `merge_activity(p_user_id, p_surviving_id, p_merged_id, p_expected_versions, p_merged_fields, p_proposal_id, p_confirmation_message_id)` | Locks the proposal (for Tier B) and the activity pair using plan-first/activity-id order, then checks whether this is an already-completed retry for the exact proposal and authenticated user — in which case it returns current state immediately. For all other requests, validates that the proposal is pending and unexpired (Tier B only), verifies the later user message whose normalized text equals `expected_phrase` (Tier B), rejects a `p_surviving_id` not first-created by `(created_at, id)`, revalidates links and exact member versions across the complete logical group — survivor, still-active members, and members already merged into it — transfers any plan link, applies fields/audit, and marks this pair merged against the proposal, consuming it atomically only when it is the final pair of the group. Stale versions raise `40001`; invalid state raises `22023`. Tier A passes null confirmation args; the two nullable confirmation args are injected by trusted orchestration, not the model |
| `unmerge_activity(p_user_id, p_merged_id, p_expected_versions, p_rederived_fields)`                                                      | Derives the survivor and audited prior link, uses the same plan-first lock order, verifies versions and current post-merge link ownership, then restores both link directions when required, moves the merge entry from `merged_from` to append-only `merge_history`, clears merge state, and applies re-derived fields. Version mismatch raises `40001`; a conflicting later plan reassignment raises `22023`; an already-active target is an idempotent return                                                                                                                                                                                                                                                                                                                                                                                       |
...
2. **Recompute over `[rebuild_from, today]`** on merge and unmerge, where
   ...
   symmetric rule.
3. **Retrieve every active activity in the rebuild window.** The current endpoint
   loads candidate activities via `repo.list_activities(..., limit=500)`, which
   orders by `activity_date desc` and caps at 500 rows. For an athlete with more
   than 500 active activities in `[rebuild_from, today]`, older rows within that
   window are silently dropped from `daily_tss`, corrupting the rebuilt snapshots.
   Recomputation must use a dedicated, unbounded, keyset-paginated query matching
   the `list_dedup_candidates` pattern — not the display-oriented 500-row cap.
...
| `tests/python/test_activity_dedup.py` (unmerge)    | Re-derivation is order-independent: in a three-member group, unmerging the **middle** member preserves the remaining member's contribution — the case a `pre_merge` snapshot restore would corrupt; and the retired entry lands in append-only `merge_history` rather than being lost                                                                                                                                                                                                                                                                                                                                                             |
...
| `tests/python/test_supabase_repo.py`               | RPC wrapper arguments; active filtering scope; `list_dedup_candidates` excludes the supplied id, has no 50-row cap, and still surfaces a duplicate lying beyond 50 unrelated activities; backlog discovery keyset-paginates; `get_activity` and `list_synced_intervals_keys` remain unfiltered                                                                                                                                                                                                                                                                                                                                                    |
| `tests/python/test_supabase_db.py`                 | RPC invariants reject tenant/target/cycle/version/link failures and a reversed survivor, including the equal-`created_at` id tie-break; concurrent merges force re-read; a three-member group merges as two transactions with an unmerge of an earlier member forcing `40001`; plan links transfer/restore safely; Tier-B proposal/message verification and consumption are atomic, including a three-member group whose consent survives the first pairwise merge and is consumed only on the last, and phrase normalization (padding, case, and internal whitespace) and a near-miss phrase that must be rejected; exact retries are idempotent |
| `tests/python/test_api.py`                         | Detection excludes self/row caps and precedes matching; merged-away rows reject generic updates; merge re-scores exact ids; Tier B rejects invalid proposal/message states and atomically consumes valid proof; ingestion passes the existing, first-created activity as p_surviving_id and the newly persisted duplicate as p_merged_id                                                                                                                                                                                                                                                                                                          |
| `tests/python/test_engine.py`                      | `get_load_snapshot_on_or_before` seeds a window correctly; merge and unmerge start at the minimum pre/post member date; the 90-day horizon is evaluated against that same date; a merge that commits without its rebuild leaves `load_rebuild_pending` and the next rebuild starts from the earliest outstanding `rebuild_from`; re-running a completed rebuild is a no-op; and a rebuild window with more than 500 activities still includes older rows in the rebuilt snapshots — the query must be unbounded, not capped at the display limit                                                                                              |
