...
export async function planSpecialistDelegation(options: {
  ...
  const planner = new Agent({
    ...
    instructions: [
      ...
      formatDataBlock("availableSpecialists", SPECIALISTS),
      formatDataBlock("durableContext", options.durableContext),
      formatDataBlock("coachingMemory", options.coachingMemory),
      formatDataBlock("athleteContext", options.athleteContext),
    ].join("\n"),
  });
