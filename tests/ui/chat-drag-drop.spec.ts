...
test.describe("composer drag-and-drop (#161)", () => {
  test("dropping a supported image attaches a chip and does not navigate", async ({
    page,
  }) => {
  ...
  test("dropping an unsupported file surfaces an error and attaches nothing", async ({
    page,
  }) => {
    await mockAuthenticatedSession(page);
    await page.goto("/");
    ...
    await expect(
      page.getByText(
        /image, GPX, FIT, TCX, and ZIP attachments are supported/i,
      ),
    ).toBeVisible();
    await expect(page).toHaveURL(/\/$/);
  });
