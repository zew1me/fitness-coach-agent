  ...
  it("describes every write tool's exact key shape, marking optional keys with '?'", () => {
    ...
    expect(proposedUpdateToolShapeHints).toBe(
      "save_activity_from_text {activity_id?, text}; " +
        ...
        "generate_training_plan {goal_id?, title?, training_model?, workouts?: [{description?, phase_name?, sport?, target_distance_meters?, target_duration_minutes?, target_tss?, title?, workout_date?, workout_type?}]}; " +
