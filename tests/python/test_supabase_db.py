...
async def test_replace_intervals_connection_rpc_rejects_empty_required_fields(
    ...
    with pytest.raises(APIError, match="user_id is required"):
        client.rpc(
    ...
    with pytest.raises(APIError, match="intervals_athlete_id is required"):
        client.rpc(
            "replace_intervals_connection",
            {
    ...
    with pytest.raises(APIError, match="access_token_ciphertext is required"):
        client.rpc(
            "replace_intervals_connection",
            {
                "p_user_id": unique_user,
